...
function setProfileBroken({ message }) {
  console.error({ header: 'Profile error', body: message });
  disableSaving = true;
}
...
function saveConfig({ profileName, identifier, dump, callback }) {
  ...
  if (!config || !config.info || !config.info.fileName) {
    ...
    console.error({ header: 'Error', body: error });
    return callback(error);
  }
...
function navigateToDataDir() {
  ...
  switch (process.platform) {
    ...
    case 'android':
      return console.error('Android not yet supported.');
    default:
...
function saveActiveName({ callback }=()=>{}) {
  ...
  if (!base || !base.fileName) {
    ...
    console.error(error);
    console.error({ header: 'Profile save error', body: error });
    return callback({ error });
  }
...
function createAllProfileConfigs({ profileName, showAlertOnFail=true, onComplete }) {
  ...
  fs.lstat(profileDir, (error, stats) => {
    if (error) {
      ...
      showAlertOnFail && console.error(message);
      onComplete({ message });
    }
    else {
